...
export class DomainEvents {
  ...
  public static async publishEvents(id: ID, logger: Logger, correlationId?: string): Promise<void> {
    const aggregate = this.findAggregateByID(id);

    if (aggregate) {
      logger.debug(
        `[${aggregate.domainEvents.map((event) => event.constructor.name)}] published ${aggregate.id.value}`,
      );
      await Promise.all(
  ...
  private static async publish(event: DomainEvent, logger: Logger): Promise<void> {
